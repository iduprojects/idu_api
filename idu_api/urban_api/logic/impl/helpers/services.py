...
async def add_service_to_db(conn: AsyncConnection, service: ServicesDataPost) -> ServiceDTO:
...
async def put_service_to_db(conn: AsyncConnection, service: ServicesDataPut, service_id: int) -> ServiceDTO:
    """Put service object."""
...
async def patch_service_to_db(
    ...
    for k, v in service.model_dump(exclude_unset=True).items():
        if k == "service_type_id":
            ...
            if service_type is None:
                raise EntityNotFoundById(service.service_type_id, "service type")
        elif k == "territory_type_id" and service.territory_type_id is not None:
...
async def delete_service_in_db(conn: AsyncConnection, service_id: int) -> dict:
...
async def get_service_with_territories_by_id_from_db(
    conn: AsyncConnection, service_id: int
