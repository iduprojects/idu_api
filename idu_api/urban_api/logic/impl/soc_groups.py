...
from idu_api.urban_api.logic.impl.helpers.soc_groups import (
    ...
    get_social_groups_by_social_value_id_from_db,
...
class SocGroupsServiceImpl(SocGroupsService):
    ...
    async def get_social_groups_by_social_value_id(self, soc_value_id: int) -> list[SocGroupDTO]:
        async with self._connection_manager.get_ro_connection() as conn:
            return await get_social_groups_by_social_value_id_from_db(conn, soc_value_id)
    ...
    async def add_service_type_to_social_group(
        ...
        async with self._connection_manager.get_connection() as conn:
            return await add_service_type_to_social_group_to_db(conn, soc_group_id, service_type)

    async def add_service_type_to_social_value(
        self, soc_value_id: int, service_type_id: int
    ) -> SocValueDTO:
        async with self._connection_manager.get_connection() as conn:
            return await add_service_type_to_social_value_to_db(conn, soc_value_id, service_type_id)
    ...
    async def get_social_value_indicator_values(
        ...
        async with self._connection_manager.get_ro_connection() as conn:
            return await get_social_value_indicator_values_from_db(
                conn, soc_value_id, territory_id, year, last_only
            )
    ...
    async def add_social_value_indicator_value(
        ...
        async with self._connection_manager.get_connection() as conn:
            return await add_social_value_indicator_value_to_db(conn, soc_value_indicator)

    async def put_social_value_indicator_value(
        self, soc_value_indicator: SocValueIndicatorValuePut
    ) -> SocValueIndicatorValueDTO:
        async with self._connection_manager.get_connection() as conn:
            return await put_social_value_indicator_value_to_db(conn, soc_value_indicator)
    ...
    async def delete_social_value_indicator_value_from_db(
        ...
        async with self._connection_manager.get_connection() as conn:
            return await delete_social_value_indicator_value_from_db(
                conn, soc_value_id, territory_id, year
            )
