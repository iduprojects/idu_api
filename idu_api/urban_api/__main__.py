...
def main(  # pylint: disable=too-many-arguments
    ...
    if __name__ in ("__main__", "idu_api.urban_api.__main__"):
        if debug:
            try:
                uvicorn.run("idu_api.urban_api:app", host=host, port=port, reload=True, log_level=logger_verbosity.lower())
            except:  # pylint: disable=bare-except
                print("Debug reload is disabled")
                uvicorn.run("idu_api.urban_api:app", host=host, port=port, log_level=logger_verbosity.lower())
        else:
            uvicorn.run("idu_api.urban_api:app", host=host, port=port, log_level=logger_verbosity.lower())
    else:
