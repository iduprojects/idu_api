...
from idu_api.urban_api.dto import ScenariosUrbanObjectDTO, UrbanObjectDTO
...
class ScenariosUrbanObject(BaseModel):
    ...
    scenario_id: ShortScenario
    public_urban_object_id: int = Field(..., description="urban object identifier in public schema", examples=[1])
    physical_object: PhysicalObjectsData
    object_geometry: ObjectGeometries
    service: ServicesData | None

    @classmethod
    def from_dto(cls, dto: ScenariosUrbanObjectDTO) -> "ScenariosUrbanObject":
        return cls(
            urban_object_id=dto.urban_object_id,
            scenario_id=ShortScenario(id=dto.scenario_id, name=dto.scenario_name),
