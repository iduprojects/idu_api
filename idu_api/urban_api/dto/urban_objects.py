...
@dataclass
class ScenariosUrbanObjectDTO:  # pylint: disable=too-many-instance-attributes
    urban_object_id: int
    scenario_id: int
    scenario_name: str
    ...
    physical_object_name: str | None
    physical_object_properties: dict[str, Any]
    physical_object_created_at: datetime | None
    physical_object_updated_at: datetime | None
    object_geometry_id: int | None
    territory_id: int | None
    address: str | None
    osm_id: str | None
    geometry: geom.Polygon | geom.MultiPolygon | geom.Point | None
    centre_point: geom.Point | None
    object_geometry_created_at: datetime | None
    object_geometry_updated_at: datetime | None
    service_id: int | None
    service_type_id: int | None
    urban_function_id: int | None
    urban_function_name: str | None
    ...
    def __post_init__(self) -> None:
        if isinstance(self.centre_point, dict):
            self.centre_point = geom.shape(self.centre_point)
        if self.geometry is None and self.centre_point is not None:
            self.geometry = self.centre_point
        if isinstance(self.geometry, dict):
            self.geometry = geom.shape(self.geometry)
